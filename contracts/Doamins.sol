...
contract Domains is ERC721URIStorage{
    ...
    constructor(string memory _tld) payable ERC721("Nova Name Servive", "NNS") {
        tld = _tld;
        console.log("The domain contrct has been deployed!");
        owner = payable(msg.sender);
    }

    function price(string memory name) public pure returns (uint256) {
        uint256 len = StringUtils.strlen(name);
        require(len > 0, "Domain length must more than 0!");
        if(len <= 3) return 0.001 * 10 ** 18;
        else if (len > 3 && len < 5) return 0.0001 * 10 **18;
        else return 0.00001 * 10 **18;
    }
    ...
    function register(string calldata name) public payable {
        ...
        console.log("%s has register domain %s!", msg.sender, name);
    ...
    function changeOwner(address payable _new) public onlyOwner {
        owner = _new;
    }
}